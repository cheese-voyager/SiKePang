...
} from "../services/distribusiService";
...
export default function DistribusiPage() {
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm("Apakah Anda yakin ingin menghapus data distribusi ini?")
    ) {
      try {
  ...
  const columns = [
      ...
      render: (row) => (
        <div>
          <p className="font-semibold text-plantation-900 flex items-center gap-1.5">
            <MapPin size={13} className="text-plantation-400" />
            {row.tujuan}
          </p>
          <p className="text-xs text-plantation-500 mt-0.5 flex items-center gap-1">
            <Package size={11} /> {row.detailDistribusi?.length || 0} Jenis
            Komoditas
          </p>
        </div>
      ),
    },
    {
      header: "Petani",
      render: (row) => (
        <p className="text-plantation-700 font-medium">{row.namaPetani}</p>
      ),
    },
    {
      header: "Tanggal",
      render: (row) => (
        <span className="text-plantation-600 flex items-center gap-1">
          <Calendar size={13} className="text-plantation-400" />
          {row.tanggalDistribusi
            ? new Date(row.tanggalDistribusi).toLocaleDateString("id-ID")
            : "-"}
        </span>
      ),
    },
    {
      ...
      render: (row) =>
        isPetugas ? (
          ...
          <span
            className={`text-xs font-bold px-2 py-1 rounded-full ${statusColors[row.status] || "badge-secondary"}`}
          >
            {row.status}
          </span>
        ),
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 rounded-lg hover:bg-plantation-100 text-plantation-600 transition-colors"
          >
            <Eye size={15} />
          </button>
          {isAdmin && (
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <Header
          ...
          subtitle="Kelola dan pantau distribusi pangan ke berbagai tujuan"
        />
        {isAdmin && (
          <motion.button
      ...
      {loading ? (
        ...
        <DataTable
          columns={columns}
          data={filtered}
          searchPlaceholder="Cari tujuan, petani, atau status..."
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, petaniId: e.target.value })
              }
              required
            >
          ...
          <div className="p-3 bg-plantation-50 rounded-xl space-y-3">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({ ...formData, komoditasId: e.target.value })
                }
                required
              >
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-semibold text-plantation-900">
                  {selectedDist.namaPetani}
                </p>
              </div>
              <div>
                <p className="text-xs text-plantation-500">Tanggal</p>
                <p className="font-semibold text-plantation-900">
                  {selectedDist.tanggalDistribusi
                    ? new Date(
                        selectedDist.tanggalDistribusi,
              ...
              <div>
                ...
                {isPetugas ? (
                  <select
                    ...
                    onChange={(e) =>
                      handleUpdateStatus(selectedDist.id, e.target.value)
                    }
                  >
                    <option value="PENDING">PENDING</option>
                    <option value="DIPROSES">DIPROSES</option>
                  ...
                  <span
                    className={`text-xs font-bold px-2 py-1 rounded-full ${statusColors[selectedDist.status] || "badge-secondary"}`}
                  >
                    {selectedDist.status}
                  </span>
                )}
            ...
            <div className="mt-4 border-t border-plantation-100 pt-4">
              <p className="text-xs font-bold text-plantation-800 uppercase tracking-wide mb-2">
                Daftar Komoditas Dikirim
              </p>
              <div className="bg-plantation-50 rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {selectedDist.detailDistribusi &&
                    selectedDist.detailDistribusi.length > 0 ? (
                      selectedDist.detailDistribusi.map((item, i) => (
                        <tr
                          key={i}
                          className="border-b border-plantation-100/50 last:border-0"
                        >
                          <td className="px-4 py-2 font-medium text-plantation-900">
                            {item.namaKomoditas}
                          </td>
                          <td className="px-4 py-2 text-right text-plantation-700">
                      ...
                      <tr>
                        <td
                          colSpan="2"
                          className="px-4 py-3 text-center text-plantation-500 text-xs"
                        >
                          Tidak ada detail komoditas.
                        </td>
                      </tr>
                    )}
                  </tbody>

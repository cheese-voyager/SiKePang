...
import { Plus, Edit3, Trash2, Users, MapPin, Shield, UserCircle } from 'lucide-react';
import Header from '../components/Header';
import DataTable from '../components/DataTable';
import Modal from '../components/Modal';
import { getPengguna, createPengguna, updatePengguna, deletePengguna } from '../services/penggunaService';
...
export default function PenggunaPage() {
  ...
  const [formData, setFormData] = useState({
    nama: '', email: '', password: '', role: 'PETANI', kelompokTani: '', alamat: '', nomorTelepon: ''
  });
  ...
  const handleSearch = (query) => {
    ...
    setFiltered(penggunaList.filter(p =>
  ...
  const openAdd = () => {
    setEditData(null);
    setFormData({ nama: '', email: '', password: '', role: 'PETANI', kelompokTani: '', alamat: '', nomorTelepon: '' });
    setModalOpen(true);
  };
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 ${row.role === 'ADMIN' ? 'bg-gradient-to-br from-red-400 to-red-600' : row.role === 'PETUGAS' ? 'bg-gradient-to-br from-blue-400 to-blue-600' : 'bg-gradient-to-br from-plantation-400 to-plantation-600'}`}>
            {row.nama?.charAt(0) || 'U'}
          </div>
          <div>
    ...
    },
    {
      header: 'Role',
      render: (row) => (
        <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-2 py-1 rounded-full uppercase ${row.role === 'ADMIN' ? 'bg-red-100 text-red-700' : row.role === 'PETUGAS' ? 'bg-blue-100 text-blue-700' : 'bg-plantation-100 text-plantation-700'}`}>
        ...
        row.role === 'PETANI' ? (
          <div className="flex flex-col gap-1">
            ...
            <span className="inline-flex items-center gap-1.5 text-plantation-600 text-[11px]">
              ...
              <span className="max-w-[150px] truncate">{row.alamat || '-'}</span>
            </span>
          </div>
        ) : (
    ...
    {
      header: 'Tgl Dibuat',
      render: (row) => row.createdAt ? new Date(row.createdAt).toLocaleDateString('id-ID') : '-'
    },
    {
      header: 'Aksi',
      render: (row) => (
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <Header title="Data Pengguna" subtitle="Kelola semua akun: Admin, Petugas, dan Petani" />
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={openAdd} className="btn-primary flex items-center gap-2 self-start">
      ...
      <div className="grid grid-cols-3 md:grid-cols-3 gap-4 mb-6">
        <div className="glass-card-light p-4 text-center border-t-4 border-plantation-500">
          <p className="text-2xl font-bold text-plantation-800">{totalPengguna}</p>
          <p className="text-xs font-semibold text-plantation-500 mt-0.5">Total Pengguna</p>
        </div>
        <div className="glass-card-light p-4 text-center border-t-4 border-red-500">
          ...
          <p className="text-xs font-semibold text-red-500 mt-0.5">Total Admin</p>
        </div>
        <div className="glass-card-light p-4 text-center border-t-4 border-blue-500">
          ...
          <p className="text-xs font-semibold text-blue-500 mt-0.5">Total Petani</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <DataTable columns={columns} data={filtered} searchPlaceholder="Cari pengguna berdasarkan nama, email, atau role..." onSearch={handleSearch} />
      )}

      {/* Add/Edit Modal */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editData ? 'Edit Data Pengguna' : 'Tambah Pengguna Baru'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-plantation-700 mb-1.5">Nama Lengkap</label>
            <input type="text" className="input-field" placeholder="Masukkan nama" value={formData.nama} onChange={(e) => setFormData({ ...formData, nama: e.target.value })} required />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-plantation-700 mb-1.5">Email</label>
              <input type="email" className="input-field" placeholder="Email login" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} required />
            </div>
            <div>
              <label className="block text-sm font-semibold text-plantation-700 mb-1.5">Role / Peran</label>
              <select className="select-field" value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value })} required disabled={!!editData}>
          ...
          {formData.role === 'PETANI' && (
            <div className="p-4 bg-plantation-50 rounded-xl space-y-4 border border-plantation-100 mt-2">
              <h4 className="text-xs font-bold text-plantation-800 uppercase tracking-wide">Data Spesifik Petani</h4>
              <div>
                <label className="block text-sm font-semibold text-plantation-700 mb-1.5">Kelompok Tani</label>
                <input type="text" className="input-field bg-white" placeholder="Masukkan nama kelompok" value={formData.kelompokTani} onChange={(e) => setFormData({ ...formData, kelompokTani: e.target.value })} required />
              </div>
              <div>
                <label className="block text-sm font-semibold text-plantation-700 mb-1.5">Alamat</label>
                <input type="text" className="input-field bg-white" placeholder="Masukkan alamat" value={formData.alamat} onChange={(e) => setFormData({ ...formData, alamat: e.target.value })} required />
              </div>
              <div>
                <label className="block text-sm font-semibold text-plantation-700 mb-1.5">No. Telepon</label>
                <input type="tel" className="input-field bg-white" placeholder="08xxxxxxxxxx" value={formData.nomorTelepon} onChange={(e) => setFormData({ ...formData, nomorTelepon: e.target.value })} required />
              </div>
            </div>
          )}

          <div className="flex gap-3 pt-4 border-t border-gray-100">
            <button type="submit" className="btn-primary flex-1">{editData ? 'Simpan Perubahan' : 'Buat Pengguna'}</button>
